...
The vendored source consists of the public Raidcore Nexus header (MIT, [`include/Nexus.h:1-6`](include/Nexus.h#L1-L6)), Dear ImGui, nlohmann/json, and the GW2 Realtime API header, each under its included license — see [`THIRDPARTY.txt`](THIRDPARTY.txt).
...
- Scan results, diffs, and in-memory JSON are held only in process memory; the addon has no settings file and writes nothing to disk except the VfxDenoiser JSON files/backups it's explicitly told to change.
...
- Addon version: `0.9.3.0`. [`src/addon/version.h:7-10`](src/addon/version.h#L7-L10)
...
| [`src/core/live_log.cpp`](src/core/live_log.cpp) | Nexus event subscribe/raise for the (currently inert) Live Log feature |
...
- [VfxD Visual Sins effect collection (companion repo, content updated by this addon)](https://github.com/Xen0phy/VfxD-Visual-Sins)

This document should be updated and the addon re-reviewed if a future version adds game-memory access, native game hooks, input automation, new network destinations, new API permissions, persistent settings/account-data caching, or a private/closed-source component.
